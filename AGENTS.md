...
<!-- TAVERN-RULES:START v1.4.0 -->
<!-- 本块由 tavern-harness/scripts/sync_tavern_rules.py 自动生成，请勿手动编辑 -->
<!-- 真源: tavern-harness · rules/ · 仓群 my-repo · v1.4.0 -->
<!-- 同步时间: 2026-09-26T01:26:10+08:00 -->
<!-- 内容哈希: sha256:05c8862dc27b7a8d（仅覆盖规则正文，不含本头部与「项目覆盖」节） -->
...
### L0-7 Git 与推送纪律 (MUST) ｜仓群实践（SillyTavern-Timelines 成文政策 + 2026-09-26 并发实证）
...
**(2) 多会话共享工作区的提交边界。** 本机的多个 agent 会话**共享同一批工作区**（跨仓群 15 仓），
而 Git 的暂存区是**全仓唯一的**，因此：

- **提交只用显式 pathspec**：`git add <具体文件>`；**禁止 `git add -A` / `-u` / `git add .`**。
- **提交前校验暂存区**：`git diff --staged --name-only` 必须**恰等于**本次要提交的文件集合；
  出现计划外文件即**中止**并排查来源。
- **自动生成区的例外只写「项目覆盖」节**：目标仓要声明本仓特例时，写规则块**末尾的「项目覆盖」节**（`PO-N`），
  **不得写进规则块正文**——正文会在下次重建时被整段抹掉。
  （该节标题是同步器的**解析边界**：规则正文里**不得**出现与该标题逐字相同的字符串。）
- **重建类工具写盘前先 `--verify`**：若目标文件**被 `.gitignore` 忽略**（本仓群存在 `agent_files_ignored` 的仓），
  "工作区不干净就拒写"的门禁**是失效的**（`git status` 看不见这类文件）→ 必须先 `--verify`，
  再**人工确认**漂移项到底是**有意手改**还是**意外覆盖**。
- **（SHOULD）并发场景优先用 `git worktree` 隔离工作树**，从源头避免共享工作区互踩；
  沿用同一工作区时，以上四条按 MUST 执行。

**原因**：

- （1）真源/工作成果不在任何会被推送的版本库里 → 迟早丢失。前作「统一规则」系统的真源仓 `Center` 与同步脚本正是因为从未推送而整体消失，是本次重建规则的直接教训。
- （2）**2026-09-25 ~ 09-26 在 24 小时内两次实证**并发会话互踩：① 一个会话把本仓例外**手改进规则块正文**，
  而该文件恰被该仓 `.gitignore` 忽略 → `git status` 看不见它 → 同步器的"脏工作区"门禁**不触发**，
  用户手写的例外会在下一次 `--apply` 时被**静默覆盖**；② 另一个会话提交自己那批文件时，把**本会话 6 分钟前
  刚写盘的产物**连带提交并推送——这次内容正确所以无害，但若当时写盘的是**半成品**（例如 `--apply` 中断），
  就会被**静默入库**。

**违反后果**：

- （1）成果丢失；或误推上游造成对外事故、污染公开 fork。
- （2）他人的在途 / 半成品改动被**静默入库并推送**（推送后不可回收）；手写的规则例外被下次重建**无声抹掉**；
  范围污染导致"这次到底改了哪些文件"无法回溯。详见坑条目 `P-18`。

---

### L0-8 子代理使用纪律（并发提速 · 模型由用户指定）(MUST) ｜本仓群专用（**2026-09-26 重写**）

**规则**：

- **用途声明**：开子代理的**首要目的是并发地推进任务、节省总耗时**；**上下文隔离只是次要收益**。
  因此**必须保证时间上的净收益**——若调度是阻塞的、或开子代理**反而更慢**，就**不要开**。
- **前提一：宿主子代理须"非阻塞 / 可并发"才允许开。** 用前先核实当前宿主的子代理语义：
  - **非阻塞（可开）**：子代理在后台跑，主脑可继续干活、完成后再取结果。
  - **阻塞（不开）**：必须**等全部子代理完成才能读取结果并继续**（典型如 Copilot 的子代理）
    → 这种情况**一律不开子代理**，改为向用户说明该限制并**询问用户要求**。
  - 注意：宿主开关会**翻转**这个语义——例如 Claude Code 设置 `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1`
    时，子代理被**强制变回前台阻塞**，此时按"不开"处理。
- **前提二：模型由用户指定，本规则不写死。** 每次要用子代理前**停下询问用户用哪个模型 / 代理**
  （用户已在同一轮预先指定者除外）。
  - 宿主若支持逐次指定就用逐次指定；**先核实"可指定性"，别假设**。以 Claude Code 为例：
    逐次调用可传 `model`，解析顺序为「逐次参数 > agent 定义 frontmatter 的 `model` >
    `CLAUDE_CODE_SUBAGENT_MODEL` 环境变量 > 主对话模型」，可选值为 `sonnet` / `opus` / `haiku` / `fable` /
    完整模型 ID / `inherit`（依据：官方文档 <https://code.claude.com/docs/en/sub-agents>，2026-09-26 核对）。
  - 宿主**不支持**按调用指定模型时，如实说明后再按用户要求决定是否仍要开。
  - **不要把任何具体模型名写进本规则**——写死了会在换宿主 / 换端点时立刻变成过时条文（`P-3` 同形）。
- **并发与失败处理**：
  - 并发数**不得超过宿主的并发上限**；具体并发数**由宿主默认与用户裁定**，本规则**不写死数值**。
  - 子代理任务必须**自包含**：明确检索范围、具体问题、期望输出。
  - 失败时**先落盘部分结果再重试**，避免中断后中间结果丢失。

**原因**：

- 子代理的价值在**并发省时**；若为了"上下文隔离"而付出更长的**墙钟时间**，就本末倒置。
- 阻塞式宿主会让"并发"退化成串行等待：不但不省时，还额外付出调度与上下文重建成本 → 故前提一是**硬门槛**。
- 本机（Claude Code）的子代理默认**后台并发**（文档记默认并发上限 20、嵌套 3 层，可用
  `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` 调整）、且**支持逐次指定模型**，故本仓群**可以**用子代理；
  但用哪个模型由用户按当下端点与成本决定。
- 并发上限不写死的原因：宿主上限与端点限流都会变，写死的数字会变成过时条文（`P-3`）。

**违反后果**：在阻塞式宿主上盲目开子代理 → 主脑空等，总耗时反而更长；模型写死 → 换宿主 / 换端点后条文立刻失效且无人执行；子代理任务不自包含 → 只拿到泛泛结果，返工重来。

> **修正记录**：
> - 前作 v1.0.0 §5 写 `gemini-3-flash-preview`、§9 又写 `claude-sonnet-4`，自相矛盾。
> - 本仓 v1.1.0–v1.3.0 曾写「子代理一律用能力较低的小模型，当前指定某个免费端点模型（含具体模型 ID）」，
>   并配套一条**自写的子代理模型门禁**。该门禁已按用户 2026-09-26 裁定**整体删除**（各仓恢复宿主默认），
>   模型指定随之无执行装置 → 本条重写为「**模型由用户指定 · 用途为并发提速 · 阻塞式宿主不开**」。
...
### P-18 并发会话共享同一工作区 → 提交边界互相污染（且 gitignore 让脏门禁失效）

**案例（2026-09-25 ~ 09-26 实测，24 小时内两次）**：本机长期同时开着多个 agent 会话，共享同一批工作区。

1. **手改进了会被重建抹掉的地方**：一个会话把某仓的例外条款**直接加进了规则块正文**。
   该文件恰被该仓 `.gitignore` 忽略 → **`git status` 看不见它** → 同步器的"工作区不干净就拒写"门禁
   **完全不触发** → 下一次 `--apply` 会把用户手写的例外**静默覆盖**。
   （该内容本应写进块内「项目覆盖」节——同步器**保留**那一节，且块头的生成注释里就写着这句话。）
2. **顺手提交了别人的在途产物**：另一个会话在 6 分钟后提交自己那批文件时，把**本会话刚写盘 6 分钟的
   规则块**连带提交并推送。这次内容正确所以无害；但若当时写盘的是一份**半成品**（例如 `--apply` 中途），
   就会被**静默入库并推送**。

**真因**：Git 的**暂存区是全仓唯一的**，`git add -A` / `-u` 的语义是"所有已知改动"，
在共享工作区里天然会捎带**其他写者**的在途改动；而重建类工具的"脏工作区"门禁依赖 `git status`，
对**被 `.gitignore` 忽略的文件**完全失效——**门禁看着是绿的，实际没门**。

**为什么与 P-4 / P-17 不同**：P-4 悬挂的是**文件引用**、P-17 悬挂的是**资料索引**，
两者的失效模式都是"**读**不到"（静默但只损失自己）；本条的失效模式是"**写**坏了别人的东西"——
读得到、也不报错，但**别人的内容被覆盖或被误提交**。

**防护**：见 `L0-7` 的「**(2) 多会话共享工作区的提交边界**」——提交只用**显式 pathspec**、
提交前校验暂存区**恰为目标文件**、自动生成区的特例只写「项目覆盖」节、
重建类工具写盘前先 `--verify` 再人工确认漂移项、并发场景优先用 `git worktree` 隔离工作树。
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| v1.4.0 | 2026-09-26 | `L0-7` 新增「**(2) 多会话共享工作区的提交边界**」（24 小时内两次并发实证：手改块正文被静默覆盖、别人的在途产物被捎带提交）；`L0-8` 重写为「**子代理使用纪律（并发提速 · 模型由用户指定）**」——删掉写死的模型名，新增「阻塞式宿主不开」「用前问用户指定模型」「用途＝并发省时」三条；新增 **P-18** 收录并发互踩事故 |
